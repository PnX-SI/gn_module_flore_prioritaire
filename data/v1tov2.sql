...
SELECT indexap, surfaceap, topo_valid, altitude_sig, altitude_sig, frequenceap, 'Remarques : ' || COALESCE(remarques,'none'), indexzp, ref_nomenclatures.get_id_nomenclature('TYPE_PHENOLOGIE',t_apresence.codepheno),(total_steriles+total_fertiles) as total_min,(total_steriles+total_fertiles) as total_max,the_geom_2154, st_transform(the_geom_2154,4326), st_transform(the_geom_3857,4326)
...
SELECT indexap,'true',ref_nomenclatures.get_id_nomenclature('TYPE_PERTURBATION',cor_ap_perturb.id_nomenclature)
FROM florepatri.cor_ap_perturb;

---------------------------------------------------------------------------
--Insertion de l'attribut physionomie dans la table taxonomie.bib_attributs
---------------------------------------------------------------------------

insert into taxonomie.bib_attributs (nom_attribut, label_attribut, obligatoire, desc_attribut, type_attribut,type_widget, liste_valeur_attribut)
select 'physionomie','Physionomie','false','Physionomie principale du taxon','text','textarea','{"values":[' || string_agg ('"' || ph.nom_physionomie || '"',',') || ']}' from florepatri.bib_physionomies ph 
